...
function render(){
  ...
  const teamInfo=$('#teamScopeInfo');
  if(teamInfo){
    if(selectedTeam){
      const teamTotal=players.filter(p=>p.t===selectedTeam).length;
      const teamChosen=players.filter(p=>p.t===selectedTeam&&isMyChoice(profile(p.id))).length;
      teamInfo.textContent=`${selectedTeam} · ${teamTotal} giocatori · ${teamChosen} scelti`;
    }else teamInfo.textContent='Tutte le squadre';
  }
